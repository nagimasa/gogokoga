                ...
                <div class="text-gray-900">
                    ...
                    <div class=" p-6">
                        ...
                        <div class="block py-6 border-b">
                            ...
                            {{ Form::text('reqruit_title',old('reqruit_title'), ['required' => 'required', 'class' => 'rounded w-full md:w-1/2 ',]) }}<br>
                        </div>
                        ...
                        <div class="block py-6 border-b">
                            ...
                            {{ Form::select('work_type',
                                ['parttime' => 'パート', 'arbeit' => 'アルバイト', 'contract_employee' => '契約社員', 'employee' => '正社員'],'parttime',
                        ...
                        <div class="block py-6 border-b">
                            ...
                            {{ Form::text('work_in_day',old('work_in_day'), ['required' => 'required', 'placeholder' => '例）9時〜17時のうちの6時間', 'class' => 'rounded w-full md:w-1/2 ',]) }}<br>
                        </div>

                        <div class="block py-6 border-b">
                            {{ Form::label('work_in_week','１週間の勤務日数', ['class' => 'form-check-label  block font-bold']) }}
                            {{ Form::text('work_in_week',old('work_in_week'), ['required' => 'required', 'placeholder' => '例）週2日からOK', 'class' => 'rounded w-full md:w-1/2 ',]) }}<br>
                        </div>

                        <div class="block py-6 border-b">
                            {{ Form::label('fee_type','お給料設定', ['class' => 'form-check-label block font-bold']) }}
                            {{ Form::select('fee_type',
                                ['時給' => '時給', '日給' => '日給', '月給' => '月給'],'時給',
                                [ 'class' => 'rounded w-full md:w-1/2 block',])
                            }}
                        </div>

                        <div class="block py-6 border-b">
                            {{ Form::label('fee','金額', ['class' => 'form-check-label font-bold block font-bold']) }}
                            {{ Form::text('fee',old('fee'), ['required' => 'required', 'placeholder' => '例）時給選択時900円、月給選択時14万円など', 'class' => 'rounded w-full md:w-1/2 ',]) }}<br>
                        </div>

                        <div class="block py-6 border-b">
                            {{ Form::label('address','勤務地', ['class' => 'form-check-label font-bold block font-bold']) }}
                            {{ Form::text('address',old('address'), ['required' => 'required', 'placeholder' => '例）古賀市○○○1-2-3', 'class' => 'rounded w-full md:w-1/2 ',]) }}<br>
                        </div>

                        <div class="block py-6 border-b">
                            {{ Form::label('maneger_name','担当者名', ['class' => 'form-check-label font-bold block font-bold']) }}
                            {{ Form::text('maneger_name',old('maneger_name'), ['required' => 'required', 'placeholder' => '例）山田太郎', 'class' => 'rounded w-full md:w-1/2 ',]) }}<br>
                        </div>

                        <div class="block py-6 border-b">
                            {{ Form::label('maneger_name_kana','担当者名カタカナ', ['class' => 'form-check-label block font-bold']) }}
                            {{ Form::text('maneger_name_kana',old('maneger_name_kana'), ['required' => 'required', 'placeholder' => '例）ヤマダタロウ', 'class' => 'rounded w-full md:w-1/2 ',]) }}<br>
                        </div>

                        <div class="block py-6 border-b">
                            {{ Form::label('maneger_tel','担当者電話番号', ['class' => 'form-check-label block font-bold']) }}
                            {{ Form::text('maneger_tel',old('maneger_tel'), ['required' => 'required', 'placeholder' => '例）092-123-4567', 'class' => 'rounded w-full md:w-1/2 ',]) }}<br>
                        </div>

                        <div class="block py-6 border-b">
                            {{ Form::label('maneger_email','担当者メールアドレス', ['class' => 'form-check-label block font-bold']) }}
                            {{ Form::text('maneger_email',old('maneger_email'), ['required' => 'required', 'placeholder' => '例）reqruit@example.com', 'class' => 'rounded w-full md:w-1/2 ',]) }}<br>
                        </div>
